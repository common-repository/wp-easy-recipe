 ...
 * */
if(!function_exists('wer_sanitize_fields')){ 
function wer_sanitize_fields($type='',$val='')
{
	// Is this textarea
	if($type='textarea')
	{
...
function save_wp_easy_recipe_meta_box($post_id) {
	...
	foreach ($meta_box['fields'] as $field) 
	{
		...
		$new = wer_sanitize_fields($field['type'],$_POST[$field['id']]);
		if ($new && $new != $old){
...
function wer_add_custom_types_to_tax( $query ) {
...
add_filter( 'pre_get_posts', 'wer_add_custom_types_to_tax' );

/*
 * Check for add .html in url
 * */
$getoption=get_wp_easy_recipe_options();
if(isset($getoption['wpe_dothtml']) && $getoption['wpe_dothtml']=='yes'):


	//Define custom rewrite rule for add .html to recipe posts and their custom taxonomy
	add_action('init', 'wer_add_html_ext_to_custom_post_types');
	function wer_add_html_ext_to_custom_post_types() {
	...
	function wer_post_type_link_filter_function( $post_link, $id = 0, $leavename = FALSE ) {
		$post = get_post($id);
	...
	add_filter('post_type_link', 'wer_post_type_link_filter_function', 1, 3);

	/*
	 * Add .html for category and tags
	 * */

	add_filter('term_link', 'wer_term_link_filter', 10);
	function wer_term_link_filter($url) {
...
//Flush the rules
add_action('init', 'wer_taxonomy_flush_rewrite');
function wer_taxonomy_flush_rewrite() {
    global $wp_rewrite;
 ...
 * 
 * */
define('WER_PLACEHOLDER', plugins_url( '/images/placeholder.jpg' , __FILE__ ));
...
function wp_easy_recipe_edit_texonomy_field($taxonomy) {
	...
	if (wp_easy_recipe_taxonomy_image_url( $taxonomy->term_id, NULL, TRUE ) == WER_PLACEHOLDER) 
		$image_text = "";
	else
	...
	return '<script type="text/javascript">
	    jQuery(document).ready(function($) {
			...
			$(".wp_easy_recipe_remove_image_button").click(function() {
				$("#taxonomy_image").val("");
				$(this).parent().siblings(".title").children("img").attr("src","' . WER_PLACEHOLDER . '");
...
function wp_easy_recipe_save_taxonomy_image($term_id) {
	$filename = sanitize_text_field($_POST['taxonomy_image']);
    if(isset($filename))
        update_option('wp_easy_recipe_taxonomy_image'.$term_id, $filename);
}
...
function wp_easy_recipe_taxonomy_image_url($term_id = NULL, $size = NULL, $return_placeholder = FALSE) {
    ...
    if ($return_placeholder)
		return ($taxonomy_image_url != '') ? $taxonomy_image_url : WER_PLACEHOLDER;
	else
...
/*
 * Register new widget for recepy
 * */

/**
 * Add a sidebar.
 */
function wer_theme_slug_widgets_init() {
    register_sidebar( array(
        'name'          => __( 'Recipe Sidebar', 'mrwebsolution' ),
        'id'            => 'wp-easy-recipe-sidebar-1',
        'description'   => __( 'Widgets in this area will be shown on all recipe pages.', 'textdomain' ),
...
add_action( 'widgets_init', 'wer_theme_slug_widgets_init' ); 
 
/*
 * Register new tips type post
 * */
add_action('init','wer_create_tips_post_type');
function wer_create_tips_post_type(){
	$lables=array(
	'name' =>__('Recipe Tips'),
...
add_shortcode('recent_tips','wer_get_single_recent_tip'); //use [recent_tips list_page_id="TIPS_LIST_PAGE_ID"]
function wer_get_single_recent_tip($attr){
...
/* Define a sort code for get latest tips*/
add_shortcode('all_tips','wer_get_all_tips');
function wer_get_all_tips(){
global $wpdb;
//define query for get the 10 best category
 ...
 * 
 * */
function wer_get_recipe_menu_categories ($atts)
{ 
	$term_id = $atts['catid'];
...
add_shortcode('cat_menu','wer_get_recipe_menu_categories'); 
